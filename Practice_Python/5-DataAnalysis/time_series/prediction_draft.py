...
def  model_prdeiction(model, method, train_list, val_list):
    for i, (train, val) in enumerate(train_list, val_list):
        with open(f'time_series/models/{method}_window/{model}_{method}_window_{n_folds}_folds_{i}', 'rb') as f:
            ...
            df_evaluation = df_evaluation._append({'Model': f'{model}_{i}',
                                                   'MSE_val': mse_val,
                                                   'RMSE_val': rmse_val,
                                                   'MAE_val': mae_val,
                                                   'MSE_test': mse_test,
            ...
            plt.title(f'Stock price predection with {method} window {n_folds} folds_{i}')
            plt.xlabel("Time")
            ...
            print(f'{model}_{i} RMSE: '+'{:.2f}'.format(rmse_val))
            print('Baseline RMSE: {:2f}'.format(baseline_rmse))

            plt.figure(figsize=(16, 10), dpi=150)
            plt.bar([f'{model} Model', 'Baseline'], [rmse_val, baseline_rmse], color=['blue', 'green'])
...
model = 'arima'
method = 'rolling'
train_list = train_list_rolling
val_list = val_list_rolling
for i in range(len(train_list)):
    arima = model_prdeiction(model, method, train_list[i], val_list[i])